...
namespace WallSwitch.src.sql
{
    class sqlLocal
    {
        ...
        public List<SQLData> db_GetLocalData()
        {
            ...
            using (SQLiteConnection m_dbConection = new SQLiteConnection("Data Source=" + workingDirectory + "; Version=3;"))
            {
                try
                {
                    ...
                    string Query = "SELECT * FROM 'tbl.Local'";
        ...
        public void db_AddLocalValue(int SiteID, string tags, string rating, string preview_url, string jpeg_url, int jpeg_width, int jpeg_height, string DateTimeAdded)
        {
            string workingDirectory = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\WallSwitch\\WallSwitch.db";
            ...
            using (SQLiteConnection m_dbConection = new SQLiteConnection("Data Source=" + workingDirectory + "; Version=3;"))
            {
                try
                {
                    ...
                    using (var sqlCommand = new SQLiteCommand("INSERT INTO 'tbl.Local' (ID, tags, rating, preview_url, jpeg_url, jpeg_width, jpeg_height, date_added, favorite) values (@ID, @tags, @rating, @preview_url, @jpeg_url, @jpeg_width, @jpeg_height, @date_added, @favorite)", m_dbConection))
                    {
